...
public class PlayerWallState : PlayerBaseState
{
    ...
    {
        player.AudioController.ChangeAudioState(PlayerAudioController.PLAYER_WALL);
    ...
    public override void FixedUpdate(PlayerController_FSM player)
    {
        ...
        player.Rigidbody.gravityScale = 0f;
        player.Rigidbody.velocity = GetNewPlayerPos(player);
    ...
    private Vector2 GetNewPlayerPos(PlayerController_FSM player)
    {
        ...
        if (Input.GetAxisRaw("Vertical") < 0)
        {
            ...
            newVelocity = Vector2.MoveTowards(new Vector2(0, player.Rigidbody.velocity.y), new Vector2(0, -player.WallSlideSpeed), player.WallAcceleration);
        }
        else if (Input.GetAxisRaw("Vertical") == 0)
        {
            newVelocity = Vector2.MoveTowards(new Vector2(0, player.Rigidbody.velocity.y), new Vector2(0, -player.WallFallSpeed), player.WallAcceleration);
        }
        else
        {
            newVelocity = Vector2.MoveTowards(new Vector2(0, player.Rigidbody.velocity.y), Vector2.zero, player.WallAcceleration);
        }
        newVelocity = new Vector2(newVelocity.x, Mathf.Clamp(newVelocity.y, player.MinSlideSpeed, player.MaxSlideSpeed));
        return newVelocity;
    }
}
